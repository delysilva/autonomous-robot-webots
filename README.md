...
*Obs.: em ambientes onde não há GPU, PyTorch instala versão CPU automaticamente se necessário.*
...
2. Carregue o mundo:
   ...
   ```
3. No nó do robô, selecione o controller `intelligent_navigator_controller`.

   * Certifique-se de que `intelligent_navigator_controller/robot_perception_model.pth` esteja presente.
4. Pressione ▶️ **Play**.
5. Observe o console do Webots para mensagens como:
...
* Threshold do reflexo: `dist_pred < 0.30` (m) — ajuste conforme necessidade do robô/mundo.
* Parâmetros das janelas gaussianas no reflexo: proporções e desvios (atualmente baseados em frações de `num_rays`).
